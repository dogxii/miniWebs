...
我为你整理了一份实用的代码优雅指南，包含原则、模式和实战技巧。
...
// 空值合并
// ========================================
const count = userInput ?? 0; // 只在 null/undefined 时使用默认值
const count2 = userInput || 0; // 0、''、false 都会使用默认值
```
...
const result = users
  ...
  .sort((a, b) => b.score - a.score) // 3. 按分数降序
  .slice(0, 10) // 4. 取前 10 名
  .map((u) => u.name); // 5. 只要名字
```
...
```js checklist.md#L1-50
## 命名
- [ ] 变量名是否有意义？
- [ ] 函数名是否描述了它做什么？
- [ ] 是否避免了缩写和单字母变量（除了 i, j, k 循环变量）？
...
1. **从小处开始**：不要试图一次重构所有代码，从一个函数、一个模块开始
2. **保持一致**：选择一种风格并坚持下去
3. **持续学习**：关注最新的语言特性和最佳实践
4. **团队协作**：制定代码规范，互相 Code Review
5. **实践为王**：多写、多改、多总结

优雅的代码是修改出来的，不是一次写成的。祝你写出越来越优雅的代码！ 🚀
